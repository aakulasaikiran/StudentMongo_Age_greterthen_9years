...
public class RetrieveIds {
	...
	public static void main(String[] args) throws UnknownHostException {
		...
		BasicDBObject bc=new BasicDBObject("_id", new BasicDBObject("$gt", 25));
		DBCursor find = col.find(bc);
		...
		while (find.hasNext()) {
			list.add((Integer) find.next().get("_id"));
		}
		...
		DBCursor find2 = col.find(new BasicDBObject("_id", new BasicDBObject("$in", list)));
